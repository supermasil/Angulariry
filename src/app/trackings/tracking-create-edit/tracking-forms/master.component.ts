...
export class MasterFormCreateComponent implements OnInit {
  ...
  allFruits: string[] = ['Apple', 'Apple', 'Lime', 'Orange', 'Strawberry', 'Straw1berry', 'Straw2berry', 'Straw3berry', 'Straw4berry', 'Straw5berry', 'Straw6berry', 'Straw7berry'];
  ...
  removeItem(itemIndex: string, boxIndex: number, item: string): void {
    let items = this.masterForm.get('items')['controls'][boxIndex].controls['items'].value;
  ...
  addbox() {
    (this.masterForm.get('items') as FormArray).push(this.createBox());
  }
